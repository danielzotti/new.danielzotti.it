...
export const Halloween = () => {
    ...
    function scheduleNextFlash(isFirstCall = false) {
        ...
        setTimeout(() => {
            // flash sequence
            const flashes = Math.random() < 0.25 ? 2 + Math.floor(Math.random() * 3) : 1; // sometimes multi
            for (let i = 0; i < flashes; i++) {
                setTimeout(() => {
                    const intensity = 0.6 + Math.random() * 0.5;
                    const duration = 80 + Math.random() * 200;
                    doFlash(intensity, isFirstCall ? 2000 : duration);
                }, i * (60 + Math.random() * 120)); // quick bursts
            }
            scheduleNextFlash();
        }, delay);
    }
    ...
    return <>
        {isActive && <>
            <div ref={wrapperRef} className={styles.wrapper}>
                <div className={styles.text} onClick={toggleHalloween}>Click here to end Halloween</div>
            </div>
